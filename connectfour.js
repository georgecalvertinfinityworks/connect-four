...
function placeCounter(board, column, player){
    ...
    for(let i=board.length-1 ; i>=0; i--) {
        if (board[i][column] === null){
            ...
            const currentCircle = document.getElementById("circle-row-"+i+"-column-"+column)
            console.log(currentCircle)
            currentCircle.style.backgroundColor = player;
            numberOfTurns += 1
    ...
    for (i=0; i<board.length; i++){
        for(j=0; j<board[i].length; j++){
          ...
          //  currentCircle.style.backgroundColor = "white"
            $("#circle-row-"+i+"-column-"+j).style.backgroundColor = "white"
    }
    }
